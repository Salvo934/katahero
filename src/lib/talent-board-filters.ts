...
export function matchTalentAthlete(a: TalentAthlete, f: TalentFilterState): boolean {
  ...
  if (q) {
    const blob = `${a.firstName} ${a.lastName} ${a.jerseyNumber ?? ""} ${a.club} ${a.agency} ${a.sport} ${a.role} ${a.category} ${a.status} ${a.nationality} ${a.dominantHand} ${a.heightCm} ${a.weightKg} ${a.scoutLine ?? ""} ${(a.playerNotes ?? []).join(" ")} ${a.badges.join(" ")}`
      .toLowerCase();
